...
export function InviteModal({ den, onClose }: InviteModalProps) {
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            toast.success(`Invite sent to ${trimmed}`, { description: "They'll receive a link to join." });
            setEmail("");
        } catch {
    ...
    return (
        <ModalShell onClose={onClose} maxWidth="max-w-md">
            ...
            <div className="flex items-center gap-3 mb-5">
                <div className="flex-1 h-px" style={{ background: "var(--color-border-card)" }} />
                <span className="text-xs" style={{ color: "var(--color-text-muted)" }}>or share a link</span>
                <div className="flex-1 h-px" style={{ background: "var(--color-border-card)" }} />
            </div>

            <div
                className="flex items-center gap-2 rounded-xl px-3 py-2.5 mb-3"
                style={{ background: "var(--color-input-bg)", border: "1.5px solid var(--color-input-border)" }}
            >
                <LinkIcon className="h-3.5 w-3.5 shrink-0" style={{ color: "var(--color-text-muted)" }} />
                {generatingLink ? (
                    <span className="text-xs flex-1" style={{ color: "var(--color-text-muted)" }}>Generating link…</span>
                ) : (
