...
// CRUD
const note = await createNote(userId, { content: 'First thought', tags: ['morning'] });
...
const results = await searchNotes(userId, { query: 'coffee', tags: ['morning'] });
```
...
```ts
import { exportDen, importDenState, clearDenLocalData } from '@meerkat/local-store';
